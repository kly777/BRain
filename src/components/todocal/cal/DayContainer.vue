<template>
    <div class="day-container">
        ...
        <div v-if="isFirstOfMonth(id)" class="month-label">
            {{ moment(getDateOfId(id)).format("M") }}月
        </div>
        <!-- 显示日期 -->
        <div class="day-number">
            {{ moment(getDateOfId(id)).format("D") }}
        </div>
        <!-- 如果是今天，显示“今”字 -->
        <div v-if="isToday(id)" class="today-indicator">
            今
        </div>
        <div class="background-color" :class="{ 'today-bg': isToday(id), 'focus-bg': isFocusDay(id) }">
        </div>
    </div>
</template>
